...
jupyter: quarto_eogeo_310
---

```{r}
options(reticulate.conda_binary = "/home/mark/anaconda3/bin/conda")
library(reticulate)
#use_condaenv("quarto_eogeo_310", conda = "/home/mark/anaconda3/bin/conda")
use_condaenv("quarto_eogeo_310")
```

## Intro and Objectives


Now we'll look at the second main type of data - vector data.
...
- PyGIS - [Ch 1: Spatial Data](https://pygis.io/docs/c_features.html#)
...
https://en.wikipedia.org/wiki/GIS_file_format
...
ADD INFO ON GEOPACKAGES AND MAYBE GEODATABASES
...
The basic idea is to combine the capabilites of pandas with the shapely library to allow you to work with geospatial data in a pandas-like way.
...
From the link above, I downloaded a zipped shapefile (**cb_2022_26_place_500k.zip**) for the Places in the state of Michigan. As we'll see, these are things like city and township boundaries. After uncompressing it, the resulting folder looks like this:
...
Now let's open the shapefile with GeoPandas. First we need to load GeoPandas.
...
To read a shapefile, we can use the GeoPandas `read_file()` function and pass in the name of the file with the `.shp` extension.
